...
func MockMySQLCompositePrimaryKey() *schema.Schema {
	table := &schema.Table{
		Name: "firmware_update_versions",
		Columns: []*schema.Column{
			{
				Name: "device_id",
				Type: &schema.ColumnType{
			...
			},
			{
				Name: "version",
				Type: &schema.ColumnType{
					Type: &schema.StringType{T: "varchar", Size: 100},
					Raw:  "varchar(100)",
					Null: false,
				},
			},
	...
	// Create composite primary key with device_id and version
	primaryKey := &schema.Index{
		Name:   "PRIMARY",
		Unique: true,
		Parts: []*schema.IndexPart{
			{
				SeqNo: 1,
				C:     table.Columns[0], // device_id
			},
			{
				SeqNo: 2,
				C:     table.Columns[1], // version
			},
		},
	}
	table.PrimaryKey = primaryKey
...
func TestCompositePrimaryKey(t *testing.T) {
	...
	data := MockMySQLCompositePrimaryKey()
	m := mockMux(ctx, "mysql", data, "test")
	drv, err := m.OpenImport("mysql://test")
	require.NoError(t, err)
	defer drv.Close()
	...
	require.Equal(t, 1, len(actualFiles))

	// Check that the generated file contains both composite key fields
	firmwareFile := actualFiles["firmwareupdateversion.go"]
	require.Contains(t, firmwareFile, "device_id")
	require.Contains(t, firmwareFile, "version")
	require.Contains(t, firmwareFile, "release_date")
	require.Contains(t, firmwareFile, "description")

	// Verify that both composite key fields are present and not duplicated
	deviceIdCount := strings.Count(firmwareFile, "device_id")
	versionCount := strings.Count(firmwareFile, "version")
	require.Equal(t, 1, deviceIdCount, "device_id field should appear exactly once")
	require.Equal(t, 1, versionCount, "version field should appear exactly once")
}
